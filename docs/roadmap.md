...
The project owner approved the provider-neutral design in `docs/phase-4-deployment-design.md` and ADR 0008 on 2026-09-08. Phase 4.1 is implemented locally with immutable environment and plan records, exact deployment approvals, durable attempts and verification, and a no-credential dry-run adapter. OpenTofu was approved as the provider-neutral infrastructure layer in ADR 0009 on 2026-09-08. Phase 4.2 is locally complete on a USD 0 Docker/k3d target: saved-plan validation, default-deny and fake brokers, least-privilege RBAC, an explicitly enabled Kubernetes TokenRequest broker, exact request and returned-claim validation, credential redaction, and verified teardown all pass. Phase 4.3 adds one explicitly enabled local adapter for an exact pre-created release marker; its real exercise proved approval and revision binding, short-lived credential delivery, least privilege, idempotent replay, separate target observation, verification, ambiguous-write containment, and teardown. Phase 4.4 deliberately produced a failed verification, required a distinct attempt-bound rollback approval and credential, restored only the recorded snapshot, independently verified `ROLLED_BACK`, retained recovery timing and audit evidence, and cleaned up the cluster. A later temporary hosted exercise is capped at approximately USD 5 total and requires a separate exact execution decision. Azure and persistent hosted K3s remain alternatives; no hosting provider, cloud resource, paid service, or production deployment is currently authorized.
...
every check to the exact output digest, and feeds the evidence into the campaign core. The fifth
slice persists and executes cross-family independent reviews, binds their evidence to candidate
digests, and permits only human `MARK_FAILED` reconciliation for ambiguous producer and reviewer
calls. The sixth slice binds every later iteration to a human-committed repair plan, recovers
interrupted checks and reviews without fabricating success or blindly retrying ambiguous calls, and
permits only a human to promote the exact validated winner digest. Phase 5.2 engineering is locally
complete; its real-provider vertical-slice evidence remains opt-in and separate.
...
evidence is ahead of a real model-driven vertical slice. Phase 5.3A extracted human-controlled
...
A local portfolio demonstration is now available. Moving to a hosted production-capable system is no longer a fixed phase count: provider integration, public identity and TLS, hosted PostgreSQL, artifact publication, backup/restore, protected-branch policy, monitoring, and the justified subset of Phase 5 will determine the remaining work and cost.
